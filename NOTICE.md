...
O código deste repositório (`app.py`, `odg_crop.py`, `odg_text.py`, testes e documentação) é distribuído sob a licença MIT (ver `LICENSE`).
...
| --- | --- | --- |
| **LibreOffice Draw** com **libcdr** | leitura do CDR e escrita do ODG, em processo separado | MPL-2.0 (LibreOffice, libcdr, librevenge) |
| **PyMuPDF** (MuPDF) | rasterização da prévia | AGPL-3.0 (Artifex). O uso comercial sem disponibilizar o código-fonte exige licença comercial da Artifex |
| **Streamlit** | interface web | Apache-2.0 |
| **defusedxml** | leitura segura do XML do ODG | PSF-2.0 |
...
CorelDRAW e CDR são marcas da Corel Corporation. Este projeto não é afiliado à Corel. OpenDocument (ODG) é um padrão OASIS/ISO aberto.
