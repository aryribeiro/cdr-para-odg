...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="Conversor CDR para ODG",
    page_icon="🖍️",
    layout="centered",
...
# ---------------------------------------------------------------------------
# CONVERSÃO: CDR (CorelDRAW) -> ODG (LibreOffice Draw)
...
# (odg_crop) -> texto artístico corrigido (odg_text) -> ODG entregue.
# O ODG é o produto: sai editável, com as formas, os textos e as imagens
# separados, e abre no LibreOffice Draw, no Inkscape e no Google Drive.
# A prévia na tela é um PNG gerado à parte, só para conferência.
# ---------------------------------------------------------------------------
SOURCE_EXT = ".cdr"
TARGET_EXT = "odg"
TARGET_MIME = "application/vnd.oasis.opendocument.graphics"

PREVIEW_LONG_SIDE_PX = 2000   # lado maior da PRÉVIA (o produto é o ODG, vetorial)
...
# Num ODG isso importa duas vezes: o LibreOffice mede o texto com a fonte que
# encontra no servidor (e o odg_text usa essa medida para devolver o texto à
# largura que o CorelDRAW registrou), e a mesma fonte é usada na prévia.
# static/fonts leva a mesma coleção do app libreoffice; o fontconfig e o
# SAL_VCL_FONTPATH fazem o LibreOffice enxergá-la.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ETAPA 1: CDR -> ODG COM O LIBREOFFICE (backoff exponencial, perfil isolado)
# ---------------------------------------------------------------------------
...
def convert_cdr_to_odg_file(input_path: Path, output_dir: Path):
    """CDR -> ODG com recortes de imagem e texto artístico corrigidos.
    A libcdr descarta o recorte das fotos (PowerClip/crop) e entrega a
    imagem inteira por cima do desenho; o odg_crop reaplica o retângulo do
    recorte que o próprio ODG guarda. O odg_text corrige a posição e a
    largura do texto artístico (ver o módulo). Devolve (odg_path, correções)."""
    ...
    return odg_path, fixes


# ---------------------------------------------------------------------------
# O QUE TEM DENTRO DO ODG
# Lido do próprio arquivo entregue, não de um intermediário: o ODG é um ZIP
# com content.xml (as páginas e as formas) e styles.xml (o tamanho da página).
# ---------------------------------------------------------------------------
...
def odg_report(odg_path: Path):
    ...
    pages = [p for p in content.iter(f"{{{_ODG_NS['draw']}}}page")]
    width_cm = height_cm = None
    if styles is not None:
        for props in styles.iter(f"{{{_ODG_NS['style']}}}page-layout-properties"):
            w = _cm(props.get(f"{{{_ODG_NS['fo']}}}page-width"))
            h = _cm(props.get(f"{{{_ODG_NS['fo']}}}page-height"))
            if w and h:
                width_cm, height_cm = w, h
                break

    shapes = images = texts = 0
    for el in content.iter():
        if not el.tag.startswith(f"{{{_ODG_NS['draw']}}}"):
            continue
        name = el.tag.rsplit("}", 1)[1]
        if name == "image":
            images += 1
        elif name == "frame":
            # no ODG um texto é draw:frame > draw:text-box > text:p; uma
            # imagem é draw:frame > draw:image (contada acima). O text:p NÃO
            # é filho direto do frame — procurar só no primeiro nível não acha.
            box = el.find("draw:text-box", _ODG_NS)
            if box is not None and "".join(box.itertext()).strip():
                texts += 1
        elif name in ("polygon", "polyline", "path", "rect", "circle", "ellipse",
                      "line", "custom-shape", "connector", "measure"):
            # draw:g (grupo) fica de fora de propósito: as formas de dentro
            # já são contadas uma a uma, e somar o grupo contaria duas vezes
            shapes += 1

    return {
        "pages": len(pages),
        "page_width_cm": width_cm or 0.0,
        "page_height_cm": height_cm or 0.0,
        "shapes": shapes,
        "images": images,
        "texts": texts,
    }
...
# ---------------------------------------------------------------------------
# ETAPA 2: PRÉVIA DO ODG (não é o produto — é para o usuário conferir)
# O ODG sai vetorial e editável; para mostrá-lo na tela, o LibreOffice o
# exporta para PDF e o PyMuPDF rasteriza a primeira página. A prévia mostra
# a PÁGINA INTEIRA, e não só a arte: é o que o usuário vai ver quando abrir o
# arquivo no LibreOffice Draw, folha e tudo.
# ---------------------------------------------------------------------------
def art_bbox(page):
    """Retângulo que envolve tudo que está desenhado na página (vetores,
    textos e imagens), limitado à página. Aqui serve só para informar o
    tamanho da arte na legenda — a prévia continua sendo a página inteira."""
    rect = pymupdf.Rect()
...
def render_preview(odg_path: Path, output_dir: Path):
    """Devolve (png_bytes, info da prévia). Levanta ConversionError se o ODG
    saiu vazio — o modo de falha mais comum desta conversão é a libcdr abrir
    o arquivo e não interpretar nada, entregando uma página em branco."""
    pdf_path = run_soffice_convert(odg_path, output_dir, "pdf:draw_pdf_Export", "pdf")
    try:
        doc = pymupdf.open(str(pdf_path))
    except Exception:
        st.error("❌ O desenho foi convertido, mas a prévia não pôde ser gerada.")
    ...
    try:
        ...
        if not (has_text or has_images or page.get_drawings()):
        ...
        art = art_bbox(page)
        zoom = PREVIEW_LONG_SIDE_PX / max(page.rect.width, page.rect.height)
        pix = page.get_pixmap(matrix=pymupdf.Matrix(zoom, zoom), alpha=False)
        info = {
            "art_width_cm": art.width / 72 * 2.54,
            "art_height_cm": art.height / 72 * 2.54,
            "has_text": has_text,
            "has_images": has_images,
            # nomes das fontes embutidas no PDF da prévia: prova de qual fonte
            # o LibreOffice usou de fato (ex.: "Impact" e não a substituta)
...
# PIPELINE COMPLETO
# ---------------------------------------------------------------------------
def convert_cdr_to_odg(input_file: str):
    """Converte um .cdr em ODG. Devolve (odg_bytes, preview_png, info).
    Pasta de trabalho própria por chamada e apagada no finally, mesmo em erro."""
    ...
    try:
        ...
        with get_conversion_slots():
            odg_path, fixes = convert_cdr_to_odg_file(input_path, work_dir)
            odg_bytes = odg_path.read_bytes()
            info = odg_report(odg_path)
            preview, preview_info = render_preview(odg_path, work_dir)

        info["version"] = label
        info.update(fixes)
        info.update(preview_info)
        return odg_bytes, preview, info
    finally:
...
def convert_upload_to_odg(file_name: str, file_bytes: bytes):
    """Converte com cache por conteúdo: clicar em "Baixar ODG" dispara um
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        return convert_cdr_to_odg(str(input_path))
...
def main():
    ...
    with st.spinner(f"Convertendo para {TARGET_EXT.upper()}..."):
        try:
            odg_bytes, preview, info = convert_upload_to_odg(
    ...
    detail = f"CorelDRAW {info['version']} · página {page_size}"
    conteudo = []
    if info["shapes"]:
        conteudo.append(f"{info['shapes']} formas")
    if info["texts"]:
        conteudo.append(f"{info['texts']} textos")
    if info["images"]:
        conteudo.append(f"{info['images']} imagens")
    if conteudo:
        detail += " · " + ", ".join(conteudo)
    ...
    if info["pages"] > 1:
        st.info(f"ℹ️ O desenho tem {info['pages']} páginas, todas incluídas no ODG. "
                "A prévia mostra a primeira.")
