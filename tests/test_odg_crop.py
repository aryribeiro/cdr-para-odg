...
def test_cracha_real_recupera_as_fotos():
    ...
    import app
    odg, _preview, info = app.convert_cdr_to_odg(str(PRIVATE / "cracha.cdr"))
    ...
    # a arte são dois crachás lado a lado (mais largos que altos) numa página
    # A4 em pé; o ODG entregue mantém a página, por isso os dois tamanhos
    assert info["page_height_cm"] > 29
    assert info["art_width_cm"] < 15 and info["art_height_cm"] < 12
    assert info["art_width_cm"] > info["art_height_cm"]
    # as fotos recortadas continuam sendo imagens dentro do ODG, e o arquivo
    # entregue leva os recortes gravados (Pictures/..._cropN.png)
    with zipfile.ZipFile(io.BytesIO(odg)) as z:
        recortes = [n for n in z.namelist() if "_crop" in n]
    assert len(recortes) == 2, recortes
...
def test_cracha_real_texto_dentro_da_arte():
    ...
    # o ODG entregue é que vai para o PDF: o que se mede aqui é o arquivo que
    # o usuário baixa, e não um intermediário
    odg, fixes = app.convert_cdr_to_odg_file(PRIVATE / "cracha.cdr", work)
    assert fixes["moved_texts"] > 0 and fixes["condensed_texts"] > 0, fixes
    pdf = app.run_soffice_convert(odg, work, "pdf:draw_pdf_Export", "pdf")
